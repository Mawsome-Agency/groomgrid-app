...
describe('ga4-server.ts', () => {
  ...
  // Helper to set NODE_ENV safely (it's read-only in some environments)
  ...
  describe('Integration tests - payment flow server events', () => {
    it('should track payment initiated followed by success', async () => {
      ...
      await trackPaymentInitiatedServer('user_123', 'sess_456', 'solo');
      const fetchBody1 = JSON.parse((fetch as jest.Mock).mock.calls[0][1].body);
      expect(fetchBody1.events[0].name).toBe('payment_initiated');

      await trackPaymentSuccessServer('user_123', 'inv_456', 2999);
      const fetchBody2 = JSON.parse((fetch as jest.Mock).mock.calls[1][1].body);
      expect(fetchBody2.events[0].name).toBe('payment_success');
    });
    ...
    it('should track payment initiated followed by failure', async () => {
      ...
      const fetchBody2 = JSON.parse((fetch as jest.Mock).mock.calls[1][1].body);
      expect(fetchBody2.events[0].name).toBe('payment_failed');
    });
  });
});
